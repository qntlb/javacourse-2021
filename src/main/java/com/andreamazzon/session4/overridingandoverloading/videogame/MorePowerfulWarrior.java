...
/**
 * This class extends Warrior. In particular, it overrides and overload the damage method,
 * see the comments. However, this is not always a good programming practice. It directly
 * inherits the other method  (we want no changes) and the fields impactOnWarriors and healthPoints.
 * Note that healthPoints is private, bust this is no problem because it gets not directly accessed,
 * but only through the method removeHealthPoints(double howManyPointsRemoved), which is protected.
 *
...
public class MorePowerfulWarrior extends Warrior {
	...
	/**
	 * Causes damages to a building. Overloaded because objects of the derived class can also damage buildings,
