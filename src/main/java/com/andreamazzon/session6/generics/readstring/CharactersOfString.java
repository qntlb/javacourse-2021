...
/**
 * This class implements the GenericPointer interface, In particular, it
 * implements the method next() with return type Character. The method is used
 * in order to read all the separate characters of a string, when it is called
 * iteratively.
 *
...
public class CharactersOfString implements GenericNext<Character> {// I give the type of the type returned in next()
	...
	/**
	 * It returns the character of the string whose position is identified by count,
	 * that gets then incremented by one.
	 */
	@Override
	public Character next() {
		return string.charAt(count++);// note the method of the wrapper class Character
	}
	 ...
	 */
	public boolean checkIfAtTheEnd() {
