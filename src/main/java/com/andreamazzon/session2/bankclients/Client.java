 ...
 * object which calls the method itself. This is done by returning "this". A
 * frequent use of this possibility is to enhance the way a complex object
 * (let's say, an object with many fields) is built. What we want to do is to
 * separate its construction with its representation. Here you see that we want
 * to construct an object of type Client for a bank. It has many fields, five of
 * them of type String, so if you use the constructor there's quite a big
 * probability to do mistakes: for example, one might invert the order two
 * strings are called in the constructor. To avoid this, you might want to
 * initialize the fields one at time, with different methods. What you could do
 * is to create first your object with your first method, and then to make it
 * set all the other fields, one per line. But this would be quite lengthy! What
 * you would like is the possibility to do multiple calling, all at the same
 * line, all for the same object. This way to construct an object in known as as
...
public class Client {
	...
	/*
	 * constructor: if one calls it from the outside, you might do mistakes, for
