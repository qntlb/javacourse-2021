...
/**
 * This class illustrates how enum types can be used, and some methods: enums type are typically used in
 * connection with switch statements, as shown here.
 *
...
public class DescribeCurrency {
	...
	public void describe() {
		...
		switch (getDescribedCurrency()) { // A switch based on the value of the enum type.
		...
		case POUND:
			...
			break;
		case YEN:
			System.out.println(getDescribedCurrency().ordinal() + 1 + " " + getDescribedCurrency() + ": The strongest Asian currency");
			break;
		case YUAN:
			System.out.println(getDescribedCurrency().ordinal() + 1 + " " + getDescribedCurrency() + ": an emerging currency");
			break;
		default:
