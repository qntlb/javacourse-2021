...
public class DescribeCurrency {
	...
	// which is what you do here for example (no meaningful in this case, just to
	// show you..)
	...
	public void describe() {
		// a switch is intended to select from a limited set of possibilities: ideal
		// match for enum
		...
		case EURO: // the ordinal() method returns the integer id in the list of the Enum type
			System.out.println(getDescribedCurrency().ordinal() + 1 + " " + getDescribedCurrency()
		...
		case YUAN:
			System.out.println(
					getDescribedCurrency().ordinal() + 1 + " " + getDescribedCurrency() + ": an emerging currency");
			break;
		default:
