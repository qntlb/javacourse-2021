...
/**
 * This is an abstract class for the simulation of a general stochastic process,
 * whose realizations are stored in a two-dimensional array: a row i represents
 * the realizations of the process for a fixed time t(i), whereas a column j
 * represents the path of the process for a state of the world omega(j). In
 * particular, an user might want to get all the realizations as a matrix but
 * also the paths, the simulated value at a given time and the average value at
 * given time of the general stochastic process S. The implementation of these
 * methods uses the generation of the realizations of the process. Since this is
 * based on the specific type of the process, the method taking care of it gets
 * implemented in the derived classes and is therefore abstract. However, all
 * the other methods can be implemented and are actually implemented here: their
 * implementation is general, so it would make no sense to repeat it over the
 * derived classes. Note that this class implements the interface
 * StochasticProcessSimulator: in that interface, one can see all the methods
 * that a class adhering to that interface must implement.
 *
 *
...
public abstract class StochasticProcessSimulator implements StochasticProcessSimulatorInterface {
	/*
	 * The matrix of realizations of S. It is private, and will be set by the
	 * protected method setRealizations(double[][] realizations)
	 */
	...
	/*
	 * This method generates the realizations of the process: it is process
	 * specific, so it is abstract
	 */
	 ...
	 */
	protected StochasticProcessSimulator(double initialValue, int numberOfSimulations, int lastTime) {
	...
	/**
	 * It sets the field realizations to a specific value. It will be called in
