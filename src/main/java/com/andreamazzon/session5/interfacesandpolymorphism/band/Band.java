 ...
 * method that makes all the instruments playing some notes. Note that all the objects get upcasted to
 * the interface they are implementing when tune is called.
 *
...
public class Band {
	/**
	 * Polymorphism: at compilation time, it doesn’t care about the type of the instrument, so we can
	 * give it directly an Instrument reference. In this way, when we add new types to the system it
	 * still works right, without any need to change the code.
	 */
